...
import { fetchReadme, getAIExplanation } from '../../services/apiClient';
...
const RepoModal = ({ repo, onClose }) => {
    ...
    const loadData = async () => {
      ...
      watchdogRef.current = setTimeout(() => {
        if (isMounted) {
          ...
          setErrorDetails('All data sources timed out. Check your internet connection.');
          setLoadingReadme(false);
        }
      }, 25000);

      try {
        setDebugStatus('FETCHING_DOCUMENTATION');
  ...
  }, [repo.full_name, repo.owner.login, repo.name, handleExplain]);
